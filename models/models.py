...
from models.fusion_heads import VisualSemanticMemoryScorer
...
class ASDiffusionBackbone(nn.Module):
    def __init__(
        ...
        if self.use_semantic_memory:
            self.visual_memory_scorer = VisualSemanticMemoryScorer(
                ...
                tau_err=tau_err,
                rho_err=rho_err,
                k_max=error_candidate_max_k,
                lambda_self=topo_lambda_self,
    ...
    def configure_semantic_prototypes(
        self,
        step_prototypes,
        error_prototypes,
        step_node_ids=None,
        predecessor_edges=None,
    ):
        if not self.use_semantic_memory:
            raise RuntimeError("Semantic memory is not enabled in this backbone.")
        self.visual_memory_scorer.configure_prototypes(
    ...
    def gtg2vid_loss(self, samples):
        ...
        return {"action_ce_loss": action_ce_loss}
    ...
    def forward_with_aux(self, x, labels=None):
        ...
        if self.use_visual_memory or self.use_semantic_memory:
            ...
            action_logits = self.action_head(features)
            frame_features = features
            aux = {"backbone_features": features.transpose(1, 2)}

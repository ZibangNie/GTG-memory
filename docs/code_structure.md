...
- `scripts/run_available_egoper_pipeline.sh`
  - AutoDL-oriented full available-only pipeline.

## Known Structural Debt

- `runner.py` mixes configuration, IO, model construction, DP, ERM, metrics, TensorBoard logging, and debug dumping.
- Experiment configs still hard-code AutoDL paths such as `/root/autodl-tmp/...`.
- Historical checkpoints and logs are still present and should be archived or moved only after useful metrics are extracted.
- `notes/semantic_memory_impl.md` is empty.
- Semantic-memory and ERM experiment variants need a single canonical report generator before more experiments are added.

# Compare EgoPER baseline and visual-memory runs.
#
# The optional task_list_json argument limits the report to ready_tasks and
# produces Markdown, CSV, and JSON summaries.
import argparse
import csv
import json
import re
...
def write_markdown(path: Path, rows, base_dirs, vm_dirs, summary, tasks):
    ...
    for _, label in METRIC_SPECS:
        header.extend([f"{label} (B)", f"{label} (VM)", "Delta"])
